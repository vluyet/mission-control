...
export async function dispatchMissionControlTaskToConstructor(input: ConstructorDispatchOptions) {
  ...
  const constructorRuntime = await getTaskConstructorRuntime(input.taskId);
  ...
  const externalTaskId = normalizeDispatchToken(input.externalTaskId) ?? getStableConstructorExternalTaskId(taskInfo.id);
  const idempotencyKey = normalizeDispatchToken(input.idempotencyKey) ?? createConstructorDispatchIdempotencyKey(taskInfo.id);
  ...
  if (constructorRuntime.kind === "missing_task") {
    return { ok: false as const, status: 404, message: t("api.taskNotFound"), details: { taskId: input.taskId } };
  }

  if (constructorRuntime.kind === "disabled") {
    await appendSystemExecutionLogInDb(taskInfo.id, "CONSTRUCTOR_DISPATCH_FAILED reason=constructor_disabled", "Constructor");
    return { ok: false as const, status: 409, message: t("api.constructorDispatchDisabled"), details: { code: "CONSTRUCTOR_DISABLED" } };
  }

  if (constructorRuntime.kind === "not_configured") {
    await appendSystemExecutionLogInDb(taskInfo.id, "CONSTRUCTOR_DISPATCH_FAILED reason=constructor_not_configured", "Constructor");
    return { ok: false as const, status: 409, message: t("api.constructorNotConfigured"), details: { code: "CONSTRUCTOR_NOT_CONFIGURED" } };
  }

  if (constructorRuntime.kind === "api_token_required") {
  ...
  try {
    upstreamResult = await dispatchConstructorTask({
      baseUrl: constructorRuntime.baseUrl,
      apiToken: constructorRuntime.apiToken,
      body: {
        externalTaskId,
        idempotencyKey,
        ...(sessionId ? { sessionId } : {}),
    ...
    return {
      ...
      details: {
        ...
        constructorBaseUrl: constructorRuntime.baseUrl
      }
    };
  }
