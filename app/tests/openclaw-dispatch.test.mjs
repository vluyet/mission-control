...
function startLegacyResponsesOpenClaw() {
  const requests = [];
  const server = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(chunk);
    ...
    if (req.method === "POST" && req.url === "/v1/responses") {
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ id: "responses_run_789", output_text: "Legacy /v1/responses completed and replied in comment." }));
      return;
    }
...
function startWorkspaceDispatchOpenClaw() {
  const requests = [];
  const server = http.createServer(async (req, res) => {
    const chunks = [];
    for await (const chunk of req) chunks.push(chunk);
    ...
    if (req.method === "POST" && (req.url === "/hooks/agent" || req.url === "/dispatch")) {
      res.writeHead(404, { "content-type": "application/json" });
      res.end(JSON.stringify({ ok: false, error: { message: "Not found." } }));
      return;
    }

    if (req.method === "POST" && req.url === "/workspace-dispatch") {
      res.writeHead(200, { "content-type": "application/json" });
      res.end(
        JSON.stringify({
          ok: true,
          result: {
            ...
            agentId: "builder",
            response: { id: "workspace_run_999", output_text: "Workspace dispatch completed and replied in comment." }
          }
        })
      );
      return;
...
test("owner can dispatch through legacy /v1/responses when hook and bridge paths are unavailable", async () => {
  const cookie = await signIn();
  const mock = await startLegacyResponsesOpenClaw();
  ...
  try {
    await json("/api/workspaces/current/openclaw", {
      ...
      cookie,
      body: {
        label: "Legacy Responses OpenClaw",
        baseUrl: mock.baseUrl,
        gatewayToken: "dispatch-token",
        enabled: true
      }
    ...
    const projectCreate = await json("/api/projects", {
      method: "POST",
      cookie,
      body: { name: "OpenClaw Responses Legacy Project" }
    });
    const projectSlug = projectCreate.payload?.data?.project?.slug;
    assert.ok(projectSlug);
    ...
    const taskCreate = await json(`/api/projects/${projectSlug}/tasks`, {
      ...
      cookie,
      body: {
        title: "Dispatch me through legacy responses",
        assigneeId: openclawAgent.id
      }
    });
    assert.equal(taskCreate.response.status, 201);
...
test("owner can dispatch through workspace-dispatch when hook and direct bridge paths are unavailable", async () => {
  const cookie = await signIn();
  const mock = await startWorkspaceDispatchOpenClaw();
  ...
  try {
    await json("/api/workspaces/current/openclaw", {
      ...
      cookie,
      body: {
        label: "Workspace Dispatch OpenClaw",
        baseUrl: mock.baseUrl,
        gatewayToken: "dispatch-token",
        enabled: true
      }
    ...
    const projectCreate = await json("/api/projects", {
      method: "POST",
      cookie,
      body: { name: "OpenClaw Workspace Dispatch Project" }
    });
    const projectSlug = projectCreate.payload?.data?.project?.slug;
    assert.ok(projectSlug);
    ...
    const taskCreate = await json(`/api/projects/${projectSlug}/tasks`, {
      ...
      cookie,
      body: {
        title: "Dispatch me through workspace-dispatch",
        assigneeId: openclawAgent.id
      }
    });
    assert.equal(taskCreate.response.status, 201);
